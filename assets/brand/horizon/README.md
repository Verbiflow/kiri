...
Preferred identity, concept 05. The app tile uses a restrained gunmetal fog background. The mark and wordmark remain separate vector artwork.
...
- `source/`: generated stencils, generation prompts, and the original background texture.
...
The 2048px mark/logo PNGs are rasterized from the traced vector paths. The textured background's original resolution is 1254px; the 2048px app icon scales that texture while keeping the foreground vector-sharp. The textured app SVG embeds this background PNG and retains vector foreground paths. Other SVGs and logo PDFs contain only vector artwork.
