...
- `scripts/benchmark_clients.py --kiri <release-binary> --gitui <release-binary> [--lumen <release-binary>]` compares real PTY inventory/patch visibility, keypress-to-patch navigation latency, and idle Git process counts on identical disposable fixtures. Inspect saved screens and Git guards; never interpret missing patch markers as a performance win. Re-run it before changing the README speed table.
...
- Git's builtin file system monitor is honored when `core.fsmonitor` is the boolean `true`. Hook-style monitors stay disabled because Kiri never runs commands named by repository configuration.
- Engine status requests accept `since`; an unchanged revision replies `unchanged` without the file list. `open` warms the status cache in the background.
